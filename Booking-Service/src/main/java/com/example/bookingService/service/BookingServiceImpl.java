...
import com.example.bookingService.dto.PaymentMode;
...
import com.example.bookingService.entity.RefundStatus;
import com.example.bookingService.entity.TripType;
...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingResponseDTO createBooking(BookingRequestDTO request) {
        ...
        if (request.getFlightId() == null || request.getFlightId() <= 0) {
            throw new RuntimeException("Valid flight id is required");
        }

        if (request.getTripType() == null || request.getTripType().isBlank()) {
            request.setTripType("ONE_WAY");
        }

        if (request.getMealPreference() == null || request.getMealPreference().isBlank()) {
            request.setMealPreference("NO_MEAL");
        }

        if (request.getSeatClass() == null || request.getSeatClass().isBlank()) {
            request.setSeatClass("NORMAL");
        }
        ...
        if (request.getLuggageKg() == null || request.getLuggageKg() < 0) {
        ...
        try {
            ...
            // Seat row may not exist in seat-service yet; allow booking to continue
            System.out.println("Seat hold skipped: " + e.getMessage());
        }

        try {
            // IMPORTANT: requires FlightClient with @RequestParam("count")
            flightClient.decrementSeat(request.getFlightId(), 1);
        } catch (Exception e) {
            try {
                if (seatHeld) {
                    seatClient.releaseSeat(request.getFlightId(), request.getSeatNumber());
                }
    ...
    public BookingResponseDTO cancelBooking(UUID bookingId) {
        ...
        try {
            ...
            System.out.println("Seat release skipped: " + e.getMessage());
        }

        try {
            flightClient.incrementSeat(booking.getFlightId(), 1);
        } catch (Exception e) {
            System.out.println("Flight seat increment skipped: " + e.getMessage());
        }
    ...
    public BookingResponseDTO requestRefund(UUID bookingId) {
        ...
        if (booking.getStatus() != BookingStatus.CONFIRMED) {
            throw new RuntimeException("Refund can be requested only for confirmed bookings");
        }
    ...
    public BookingResponseDTO approveRefund(UUID bookingId) {
        ...
        if (booking.getRefundStatus() != RefundStatus.REQUESTED) {
            throw new RuntimeException("No pending refund request for this booking");
        }
        ...
        // If your client method is refund(...), change this line accordingly
        paymentClient.refundPayment(paidPayment.getPaymentId());
    ...
    public List<BookingResponseDTO> getRefundRequests() {
        return bookingRepository.findAll()
                .stream()
                .filter(booking -> booking.getRefundStatus() == RefundStatus.REQUESTED)
                .map(this::mapToResponse)
                .collect(Collectors.toList());
    }
    ...
    public BookingResponseDTO updateBookingStatus(UUID bookingId, String status) {
        ...
        booking.setStatus(BookingStatus.valueOf(status.trim().toUpperCase()));
        return mapToResponse(bookingRepository.save(booking));
    }
    ...
    public FareSummaryDTO calculateFare(
        ...
        try {
            ...
            // Keep only methods that exist in your DTO
            if (flight.getBaseFare() != null) {
                flightBase = flight.getBaseFare();
            }
        } catch (Exception e) {
        ...
        if ("VEG".equals(meal)) {
            mealCharge = 100.0;
        } else if ("NON_VEG".equals(meal)) {
            mealCharge = 250.0;
        }

        String seat = seatClass == null ? "NORMAL" : seatClass.toUpperCase();
        double seatCharge = 0.0;
        if ("ECONOMY".equals(seat)) {
            seatCharge = 500.0;
        } else if ("BUSINESS".equals(seat) || "FIRST".equals(seat)) {
            seatCharge = 1500.0;
        }
    ...
    public PaymentResponseDTO startPayment(UUID bookingId, String method) {
        ...
        try {
            request.setPaymentMode(PaymentMode.valueOf(method.toUpperCase()));
        } catch (Exception e) {
    ...
    @Override
    public BookingResponseDTO completePayment(UUID bookingId, String transactionId, String status) {
        Booking booking = bookingRepository.findById(bookingId)
                .orElseThrow(() -> new RuntimeException("Booking not found"));
        ...
        if ("PAID".equalsIgnoreCase(status)) {
            booking.setStatus(BookingStatus.CONFIRMED);
            try {
                seatClient.lockSeat(booking.getFlightId(), booking.getSeatNumber());
            } catch (Exception e) {
                System.out.println("Seat lock skipped: " + e.getMessage());
            }
        } else {
            booking.setStatus(BookingStatus.CANCELLED);
    ...
    private TripType parseTripType(String value) {
        ...
        if ("ROUND".equals(normalized) || "TWO_WAY".equals(normalized) || "RETURN".equals(normalized)) {
            return TripType.ROUND_TRIP;
        }

        if ("ONEWAY".equals(normalized)) {
            return TripType.ONE_WAY;
        }
    ...
    private BookingResponseDTO mapToResponse(Booking booking) {
        ...
        dto.setRefundStatus(
                booking.getRefundStatus() == null
                        ? RefundStatus.NON_REFUNDED.name()
